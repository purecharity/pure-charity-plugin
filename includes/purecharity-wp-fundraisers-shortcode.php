...
class Purecharity_Wp_Fundraisers_Shortcode {
   ...
   * @since    1.0.0
   * @access   public
   * @var      Object    $base_class    The Base Class.
   */
  ...
  public static function init()
  {
    //if(Purecharity_Wp_Fundraisers::base_present()){
      ...
      //self::$base_class = new Purecharity_Wp_Base();
    //}
  }
  ...
  public static function last_fundraisers_shortcode($atts)
  {
    ...
    if(isset($_GET['fundraiser'])){
      ...
      $fundraisers = Purecharity_Wp_Base::api_call('external_fundraisers?' . join('&', $query_var));

      if ($fundraisers && count($fundraisers) > 0) {
        Purecharity_Wp_Base_Public::$fundraisers = $fundraisers;
        return Purecharity_Wp_Base_Public::listing_last_grid();
      }else{
    ...
    if(isset($_GET['fundraiser'])){
      ...
      if ($fundraisers && count($fundraisers) > 0) {
        ...
        if($options['grid'] == 'true'){
          return Purecharity_Wp_Base_Public::listing_grid($options);
        }else{
          return Purecharity_Wp_Base_Public::listing();
        }
      }else{
        return Purecharity_Wp_Base_Public::list_not_found();
      };
  ...
  public static function fundraiser_shortcode($atts)
  {
    ...
    if ($options['slug']) {
      $fundraiser = Purecharity_Wp_Base::api_call('fundraisers/show?slug='. $options['slug']);
      if ($fundraiser) {
        $fundraiser = $fundraiser->fundraiser;
        Purecharity_Wp_Base_Public::$fundraiser = $fundraiser;
        Purecharity_Wp_Base_Public::$options = $options;
        return Purecharity_Wp_Base_Public::show();
      }else{
        return Purecharity_Wp_Base_Public::not_found();
      }
    }
  }
  ...
  public static function fundraiser_funding_bar_shortcode($atts)
  {
    ...
    if ($options['fundraiser']) {
      $fundraiser = Purecharity_Wp_Base::api_call('fundraisers/show?slug='. $options['fundraiser']);

      if ($fundraiser) {
        $fundraiser = $fundraiser->fundraiser;
        Purecharity_Wp_Base_Public::$fundraiser = $fundraiser;
        Purecharity_Wp_Base_Public::$options = $options;
        return Purecharity_Wp_Base_Public::single_view_funding_bar();
      }else{
        return Purecharity_Wp_Base_Public::not_found();
      }
